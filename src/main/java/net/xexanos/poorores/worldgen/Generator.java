...
public class Generator implements IWorldGenerator {
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider) {
        for (PoorOre poorOre : Reference.ORES_LIST) {
            if (poorOre.getVeinRate() != 0 && poorOre.getVeinRate() != 0 && poorOre.getVeinSize() != 0 && poorOre.getDimWhiteList().contains(world.provider.dimensionId) || (poorOre.getDimWhiteList().isEmpty() && !poorOre.getDimBlackList().contains(world.provider.dimensionId))) {
